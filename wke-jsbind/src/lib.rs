...
pub fn derive_from_js(item: TokenStream) -> TokenStream {
    ...
    if let Data::Struct(data_struct) = input.data {
        for field in data_struct.fields.iter() {
            ...
            let field_identy = &field.ident.as_ref().unwrap();
            for meta in field.attrs.iter().map(|attr| &attr.meta) {
                if let Meta::Path(path) = meta {
                    ...
                    tokens.push(quote! {
                        if let Ok(prop) = value.get(stringify!(#field_identy)) {
                            obj.#field_identy = wke::javascript::FromJs::from_js(&prop)?;
                        };
                    });
                }
            }
        ...
        return quote! {
            compile_error!("FromJs can only mark struct").into()
        }
        .into();
    };

    let gen = quote! {
        impl wke::javascript::FromJs for #name {
            fn from_js(value: &wke::javascript::JsValue) -> wke::error::Result<Self> {
                if !value.is_object() {
                    return Err(wke::error::Error::JsCallException);
                }
...
pub fn derive_into_js(item: TokenStream) -> TokenStream {
        ...
        for field in data_struct.fields.iter() {
            ...
            for meta in field.attrs.iter().map(|attr| &attr.meta) {
                if let Meta::Path(path) = meta {
                    ...
                    tokens.push(quote! {
                        obj.set(stringify!(#field_identy), &self.#field_identy.into_js()?)?;
                    });
                }
            }
        }
    ...
    let gen = quote! {
        impl wke::javascript::IntoJs for #name {
            fn into_js(&self) -> wke::error::Result<wke::javascript::JsValue>
            {
                let obj = wke::javascript::JsValue::object()?;
                #(#tokens)*
...
// pub fn js(attrs: TokenStream, item: TokenStream) -> TokenStream {
//     // 解析输入的代码为一个函数
